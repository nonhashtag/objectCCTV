...
def tracker(nextidx, onlist: list, detected: list): #detected-component = [ 0, object-N, confidence, startX, startY, endX, endY ]
    ...
    for i in range(len(onlist)):
        if (onlist[i][1] == detected[1]
                and abs(onlist[i][4]-detected[3]) < 40
                and abs(onlist[i][5]-detected[4]) < 40
                and abs(onlist[i][6]-detected[5]) < 40
                and abs(onlist[i][7]-detected[6]) < 40):
        ...
        for i in range(len(onlist)):
            if (datetime.now() - onlist[i][2]).seconds >= 10 and onlist[i][1] == 7:
                if onlist[i][8] == False:
                    ...
                    #client.api.account.messages.create(to="수신자", from_="발신번호", body="Car {} Warn!".format(onlist[i][0]))
                    #client.api.account.messages.create(to="수신자", from_="발신번호", body="Car {} Warn!".format(i[0]))
                    onlist[i][8] = True
